...
![Screenshot_1](https://telegra.ph/file/e73581c2805c7610e51f6.png)

# 🟣 PocketFi AutoBot /// HPV /// V1.03 🟣

| **Возможности**                                                    | **Поддерживается**  |
|----------------------------------------------------------------|:---------------:|
| **Многофункциональный** — *выполняет все те же действия, что вы делали бы руками:*<br>**—** *Получение ежедневной награды*;<br>**—** *Выполнение задания с подпиской на Telegram*;<br>**—** *Выполнение задания с подпиской на Twitter*;<br>**—** *Сбор монет*;<br>**—** *Ожидание от 5 до 6 часов*;<br>**—** *Повторение действий через 5-6 часов*. |✅|
...
| **Proxy** — *скрипт поддерживает добавление неограниченного количества прокси-серверов. Даже если количество прокси меньше, чем количество аккаунтов, они будут равномерно распределены между всеми аккаунтами после проверки на валидность. Хотя использование прокси не является обязательным, настоятельно рекомендуется их использование для предотвращения блокировки в PocketFi (не в Telegram)* |✅|
...
- **Linux**
  ...
  - ```
    ...
    ```
  - ```
    git clone https://github.com/A-KTO-Tbl/PocketFi
    ```
  - ```
    pip3 install -r PocketFi\Core\Tools\requirements.txt
    ```
- **Windows**
  ...
  - Запустится CMD в текущей директории. Далее просто вводим следующие команды:
    - ```
      ...
      ```
    - ```
      pip install -r PocketFi\Core\Tools\requirements.txt
      ```
- **Android**
...
- **Инструкция по получению уникальной ссылки:**
  ***
  **1)** *Перейдите в [бота](https://t.me/pocketfi_bot/Mining?startapp=1295320967);*<br>
  **2)** *Введите команду **/start**;*<br>
  **3)** *Нажмите на кнопку **"Майнинг"***;<br>
...
- **Linux**
  - ```
    cd PocketFi && python3 HPV_PocketFi.py
    ```
- **Windows**
...
- **Android**
  - После подготовки скрипта на ПК и клонирования загруженного репозитория с GitHub в [Termux](https://github.com/termux/termux-app/releases), запустите скрипт следующей командой:
    - ```
      cd PocketFi && python HPV_PocketFi.py
      ```
